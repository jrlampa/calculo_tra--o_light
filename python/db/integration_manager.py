"""
    The provided code defines a Database Integration Manager for calculo_tração_light, offering unified
    database access with connection pooling, transaction management, and support for multiple database
    types like PostgreSQL, Redis, and Supabase.
...
class DatabaseConnectionManager:
    ...
    def _setup_redis_client(self, name: str, config: DatabaseConfig):
        ...
        redis_client = aioredis.from_url(config.url, encoding="utf-8", decode_responses=True)
    ...
    async def health_check(self, db_name: str) -> Dict[str, Any]:
        ...
        try:
            if config.type == DatabaseType.POSTGRESQL:
                ...
                return {"status": "healthy", "type": config.type.value, "duration": duration}

        except Exception as e:
            return {"status": "unhealthy", "type": config.type.value, "error": str(e)}
...
def setup_database_connections():
    ...
    if redis_url:
        redis_config = DatabaseConfig(type=DatabaseType.REDIS, url=redis_url)
...
async def test_connections():
    ...
    for db_name, config in db_manager.configs.items():
        ...
        if health["status"] == "healthy":
            ...
            logger.error(
                f"❌ {config.type.value} connection failed for {db_name}: {health.get('error', 'Unknown error')}"
            )
...
async def get_database_health() -> Dict[str, Any]:
    ...
    for db_name in db_manager.configs.keys():
        ...
        health_status[db_name] = {"health": health, "stats": stats}
